...
class Group:
    ...
    def add_student(self, student):
        if len(self.students) == Group.MAX_SIZE:
            raise GroupTooManyStudentsException()
...
class GroupTooManyStudentsException(Exception):

    def __init__(self):
        super().__init__(f'A group cannot contain more {Group.MAX_SIZE} students.')
...
for i in range(10):
    group.add_student(Student(f'Student {i}', f'Lastname {i}', 20, 'male'))

try:
    group.add_student(Student('Student 11', 'Lastname 11', 20, 'male'))
except GroupTooManyStudentsException as e:
    print(e)
